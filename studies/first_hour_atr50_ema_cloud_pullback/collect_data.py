#!/usr/bin/env python3
"""
Data collection for First Hour 50%+ ATR to EMA Cloud Pullback Study.

Cache-first strategy:
- Reuse daily + 5-minute CSVs from the parent study's latest run when available
  (opening_thrust_ema_cloud_pullback_1atr_trend)
- Fetch any missing tickers via DataRouter (Alpaca → Tiingo fallback)
- Incrementally append April 4-8 2026 gap if cached data ends before today
"""
...
def _get_csv_last_date(path: Path) -> str | None:
    """Return the latest date string found in a CSV's index column, or None."""
    try:
        df = pd.read_csv(path, usecols=[0], parse_dates=[0])
        if df.empty:
            return None
        last_ts = pd.to_datetime(df.iloc[:, 0], errors="coerce").dropna().max()
        if pd.isna(last_ts):
            return None
...
def _append_intraday_gap(ticker: str, intraday_path: Path, gap_start: str, gap_end: str) -> None:
    """Fetch gap_start..gap_end and append to existing intraday CSV (dedup by index)."""
    new_df = _fetch_intraday_chunked(ticker, gap_start, gap_end)
    if new_df is None or new_df.empty:
        return
    existing = pd.read_csv(intraday_path, parse_dates=["timestamp"]).set_index("timestamp")
    combined = pd.concat([existing, _normalize_ohlcv(new_df)], axis=0).sort_index()
    combined = combined[~combined.index.duplicated(keep="last")]
    _save_df(combined, intraday_path)
...
def _resolve_source_data_dir(cfg: dict, source_run_id_override: str | None) -> Path | None:
    ...
    source_study = reuse_cfg.get("source_study", "opening_thrust_ema_cloud_pullback_1atr_trend")
    source_outputs = ROOT_DIR / "studies" / source_study / "outputs"
    if not source_outputs.exists():
        print(f"WARNING: source study outputs not found at {source_outputs}")
        return None
    ...
    if not run_id:
        ...
        if not latest_file.exists():
            print(f"WARNING: LATEST.txt not found in {source_outputs}")
            return None
    ...
    print(f"WARNING: source data dir not found: {source_data}")
    return None
...
def collect(
    ...
    end = end or (datetime.now().strftime("%Y-%m-%d") if str(end_raw).lower() == "today" else str(end_raw))
    ...
    for ticker in tickers:
        ...
        # --- Check for intraday gap (parent study ends ~2026-04-03, we need through today) ---
        if intraday_path.exists():
            last_date = _get_csv_last_date(intraday_path)
            if last_date and last_date < end:
                # Next trading day after the cached data
                gap_start_dt = datetime.strptime(last_date, "%Y-%m-%d") + timedelta(days=1)
                gap_start = gap_start_dt.strftime("%Y-%m-%d")
                if gap_start <= end:
                    try:
                        _append_intraday_gap(ticker, intraday_path, gap_start, end)
                        msg = f"{ticker}: appended intraday gap {gap_start} to {end}"
                        print(msg)
                        log_lines.append(msg)
                    except Exception as exc:
                        msg = f"WARNING: {ticker} gap append failed: {exc}"
                        print(msg, file=sys.stderr)
                        log_lines.append(msg)

        # Also extend daily data if cached
        if daily_path.exists():
            last_date = _get_csv_last_date(daily_path)
            if last_date and last_date < end:
                gap_start_dt = datetime.strptime(last_date, "%Y-%m-%d") + timedelta(days=1)
                gap_start = gap_start_dt.strftime("%Y-%m-%d")
                if gap_start <= end:
                    try:
                        new_daily = DataRouter.get_price_data(
                            ticker=ticker,
                        ...
                        if new_daily is not None and not new_daily.empty:
                            existing_d = pd.read_csv(daily_path, parse_dates=["timestamp"]).set_index("timestamp")
                            combined_d = pd.concat([existing_d, _normalize_ohlcv(new_daily)], axis=0).sort_index()
                            combined_d = combined_d[~combined_d.index.duplicated(keep="last")]
                            _save_df(combined_d, daily_path)
                    except Exception as exc:
                        msg = f"WARNING: {ticker} daily gap append failed: {exc}"
                        print(msg, file=sys.stderr)
                        log_lines.append(msg)
        ...
        # --- Paths still missing: fetch fresh ---
        daily_rows = 0
        intraday_rows = 0
    ...
    print(f"\nCollection complete: {len(manifest_rows)} tickers processed")
